...
parser.add_argument(
    "--data-dir", type=pathlib.Path, help="data dir (default: project data dir)"
)
parser.add_argument(
    "--results-dir",
    type=pathlib.Path,
...
parser.add_argument(
    "--targets", nargs="+", help="prespecified targets (default: read from data dir)"
)
...
for target in progress:
    ...
    with tempfile.TemporaryDirectory(prefix=name) as tempdir:
        ...
        for layer_dir in target.iterdir():
            ...
            for file_name in ("masks.npy", "ids.csv"):
                ...
                if not src_file.exists():
                    raise FileNotFoundError(f"missing required file: {src_file}")
        ...
        shutil.make_archive(str(results_dir / name), "zip", root_dir=temp_out_dir)
