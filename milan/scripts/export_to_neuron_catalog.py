...
parser.add_argument(
    "--no-save-images", action="store_true", help="do not save top images"
)
...
for key in args.sources:
    dataset = milannotations.load(
        key, factory=datasets.TopImagesDataset, path=data_dir / key
    )
...
descriptions = {}
for key in args.sources:
    descriptions_file = results_dir / f'cache/{key.replace("/", "_")}_descriptions.csv'
    if descriptions_file.exists():
        ...
        predictions = decoder.predict(
            data[key], strategy="rerank", temperature=0.2, beam_size=50, device=device
        )
