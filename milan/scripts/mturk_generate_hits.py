...
parser.add_argument(
    "--limit", type=int, help="only generate hits for this many units (default: None)"
)
parser.add_argument(
    "--host-url",
...
parser.add_argument(
    "--no-validate-urls", action="store_true", help="do not validate urls"
)
parser.add_argument(
    "--no-display-progress", action="store_true", help="do not show progress bar"
)
args = parser.parse_args()

dataset = milannotations.load(
    args.dataset, path=args.dataset_path, display_progress=not args.no_display_progress
)
...
def generate_urls(layer: Layer, unit: int, k: int) -> Sequence[str]:
    ...
    return [f"{base_url}/{layer}/unit_{unit}/image_{index}.png" for index in range(k)]
