...
parser.add_argument(
    ...
    help='replace all "space around" phrases for this worker ' "(default: none)",
)
parser.add_argument(
    "--replace-for-worker",
...
parser.add_argument(
    "--legacy",
    action="store_true",
    help="if set, parse layer/unit from image url " "(default: use layer/unit columns)",
)
    ...
    for noun in ("space", "spaces"):
        for preposition in ("around", "along", "to", "in"):
            for article in ("a ", "an ", "the ", ""):
                ...
                replacements_by_worker.append((replace_space_around_for, phrase, ""))
            for punct in (".", ","):
