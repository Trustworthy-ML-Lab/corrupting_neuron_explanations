            ...
            *batch: Any,
            ...
            for ranks, acts, image in zip(gather, activations, images):
                for unit, rank in ranks:
                    ...
                    masked = self.pytorch_masked_image(
                        image, mask=mask, outside_bright=0.25, thickness=0
                    )
                    unmasked = self.pytorch_image(image)  # Compute last!
                    result = torch.cat(
                        [
