...
from . import renormalize, segviz, tally, upsample
...
class ImageVisualizer:
    def __init__(
        ...
        if renormalizer is None and source is not None:
            renormalizer = renormalize.renormalizer(source=source, target="byte")
        ...
        if self.data_size is not None:
            self.upsampler = upsample.upsampler(
                size, data_size, image_size=self.image_size, scale_offset=scale_offset
            )

    def heatmap(self, activations, unit=None, mode="bilinear", amax=None, amin=None):
        """
    ...
    def image(self, imagedata):
        ...
        """
        return PIL.Image.fromarray(
            self.pytorch_image(imagedata).permute(1, 2, 0).byte().cpu().numpy()
        )
    ...
    def pytorch_masked_image(
        ...
        """
        scaled_image = self.pytorch_image(imagedata).float()
        if mask is None:
            mask = self.pytorch_mask(
                activations, unit, level=level, percent_level=percent_level
            )
        ...
        border_color = torch.tensor(
            border_color, dtype=border.dtype, device=border.device
        )[:, None, None]
        if inside_color is not None:
            inside_color = torch.tensor(
                inside_color, dtype=border.dtype, device=border.device
            )[:, None, None]
        result_image = (
            (
                (scaled_image if inside_color is None else inside_color) * inside
                + border_color * border
                + outside_bright * scaled_image * outside
            )
            .clamp(0, 255)
            .byte()
        )
    ...
    def masked_delta(self, imagedata, activations, unit=None, above=None, below=None):
        """
        Visualizes the given activations, thresholded at a specified level,
        overlaid on the given image, as a PIL image.
        """
        result_image = self.pytorch_masked_delta(
            imagedata, activations, unit=unit, above=above, below=below
        )
        return PIL.Image.fromarray(result_image.permute(1, 2, 0).cpu().numpy())

    def pytorch_masked_delta(self, imagedata, delta, unit=None, above=None, below=None):
        """
        Visualizes the given activations, thresholded at a specified level,
        ...
        red, green = [
            torch.tensor(c, dtype=torch.float, device=aborder.device)[:, None, None]
    ...
    def pytorch_mask(self, activations, unit, level=None, percent_level=None):
        ...
        if unit is None:
            ...
            a = activations[unit]
        if level is None:
            level = self.level_for(activations, unit, percent_level=percent_level)
        upsampler = self.upsampler_for(a)
    ...
    def pytorch_image(self, imagedata):
        ...
        return torch.nn.functional.interpolate(
            renormalizer(imagedata).float()[None, ...], size=self.size
        )[0]
    ...
    def masked_image_grid_for_topk(self, compute, dataset, topk, k=None, **kwargs):
        def compute_viz(gather_indices, *data_batch):
            ...
            for gather_for, acts, imgt in zip(gather_indices, acts_batch, image_batch):
                for unit, rank in gather_for:
                    yield (
                        (unit, rank),
                        self.pytorch_masked_image(imgt, acts, unit)
                        .permute(1, 2, 0)
                        .cpu(),
                    )
    ...
    def individual_masked_images_for_topk(
        ...
        gt = self.masked_image_grid_for_topk(compute, dataset, topk, k=k, **kwargs)
        return [[PIL.Image.fromarray(d.cpu().numpy()) for d in row] for row in gt]

    def masked_images_for_topk(self, compute, dataset, topk, k=None, gap=5, **kwargs):
        # Example compute function:
        # def compute(image_batch):
        #   image_batch = image_batch.cuda()
        #   acts_batch = model.retained_layer(layername)
    ...
    def masked_image_grid_for_row(self, compute, dataset, unit, indexes):
        ...
        for rank in indexes:
            ...
            results.append(
                self.pytorch_masked_image(img_batch[0], acts_batch[0], unit)
                .permute(1, 2, 0)
                .cpu()[None, ...]
            )
    ...
    def masked_image_for_conditional_topk(
        self, compute, dataset, ctk, classnum, unit, k=10, gap=5
    ):
        row = self.masked_image_grid_for_row(
            compute, dataset, unit, ctk.conditional(classnum).result()[1][unit][:k]
        )
        return strip_image_from_grid_row(row, gap=gap)
...
def strip_image_from_grid_row(row, gap=5, bg=255):
    strip = torch.full(
        (row.shape[1], row.shape[0] * (row.shape[2] + gap) - gap, row.shape[3]),
        bg,
        dtype=row.dtype,
    )
