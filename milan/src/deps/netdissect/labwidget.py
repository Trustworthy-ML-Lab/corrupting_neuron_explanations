    ...
    def prop(self, name):
        ...
        if not isinstance(curvalue, Trigger):
            raise AttributeError(
                "%s not a property or trigger but %s" % (name, str(type(curvalue)))
            )
        return curvalue
...
class Widget(Model):
    ...
    def std_attrs(self):
        ...
        """
        return f'id="{self.view_id()}"' + style_attr(self.style) + data_attrs(self.data)
    ...
    def _repr_html_(self):
        ...
        json_data = json.dumps(
            {k: v.value for k, v in vars(self).items() if isinstance(v, Property)}
        )
        json_data = re.sub("</", "<\\/", json_data)

        std_widget_js = minify(
            f"""
...
class Trigger(object):
    ...
    def off(self, cb=None):
        ...
        """
        self._listeners = [
            (c, i) for c, i in self._listeners if c != cb and cb is not None
        ]
...
class capture_output(object):
    ...
    def __exit__(self, exc_type, exc_value, exc_tb):
        ...
        if exc_type:
            ...
            tbtxt = "".join(traceback.format_exception(exc_type, exc_value, exc_tb))
...
class Choice(Widget):
    ...
    def __init__(self, choices=None, selection=None, horizontal=False, **kwargs):
        super().__init__(**kwargs)
        if choices is None:
            choices = []
...
class Datalist(Widget):
    ...
    def widget_html(self):
        options = [
            f"""<option value="{html.escape(str(value))}">""" for value in self.choices
        ]
        return "".join(
            [
...
class Div(Widget):
    ...
    def print(self, *args, replace=False):
        ...
        newHTML = "<pre>%s</pre>" % " ".join(html.escape(str(text)) for text in args)
        if replace:
...
def data_attrs(d):
    if not d:
        return ""
    return "".join([' data-%s="%s"' % (k, html.escape(str(v))) for k, v in d.items()])
