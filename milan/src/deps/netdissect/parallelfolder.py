...
class ParallelImageFolders(data.Dataset):
    ...
    def __init__(
        ...
        def do_lazy_init():
            self.images, self.classes, self.class_to_idx = make_parallel_dataset(
                image_roots,
    ...
    def subset(self, indexes):
        ...
        ds.images = [copy.deepcopy(self.images[i]) for i in indexes]
    ...
    def __getitem__(self, index):
        ...
        if self.transforms is not None:
            sources = [
                transform(source) if transform is not None else source
                for source, transform in itertools.zip_longest(sources, self.transforms)
            ]
    ...
    for j, root in enumerate(image_roots):
        for path in walk_image_files(root, verbose=verbose):
            ...
            if not intersection and len(image_sets[key]) != j:
                raise RuntimeError(
                    "Images not parallel: " "{} missing from {}".format(key, root)
                )
            image_sets[key].append(path)
    if classification:
        classes = sorted(
            set([os.path.basename(os.path.dirname(k)) for k in image_sets.keys()])
        )
