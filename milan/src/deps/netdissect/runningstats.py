...
class RunningTopK:
    ...
    def add(self, data, index=None):
        ...
        if self.top_data.shape[-1] < self.next + sk:
            ...
            self.top_data[:, : self.k], self.top_index[:, : self.k] = self.result(
    ...
    def set_state_dict(self, dic):
        ...
        self.linear_index = (
            torch.from_numpy(dic["linear_index"])
    ...
    def set_state_dict(self, dic):
        ...
        for k, v in dic.items():
            if "." in k:
                ...
                subdicts[p][s] = v
        self.running_topk = {
            c: RunningTopK(state=subdicts[str(i)]) for i, c in enumerate(conditions)
        }
...
class GatherTensor:
    ...
    def add(self, index, rank, data):
        if self._grid is None:
            ...
            self._grid = torch.zeros(shape, dtype=data.dtype, device=data.device)
...
class RunningQuantile:
    ...
    def integrate(self, fun):
        ...
        for level, ff in enumerate(self.firstfree):
            if ff == 0:
                continue
            term = torch.sum(fun(self.data[level][:, :ff]) * (2.0**level), dim=-1)
            if result is None:
                result = term
            else:
                result += term
    ...
    def readout(self, count=1001, old_style=True):
        return self.quantiles(torch.linspace(0.0, 1.0, count), old_style=old_style)
...
class RunningConditionalQuantile:
    ...
    def __init__(self, r=3 * 1024, buffersize=None, seed=None, state=None):
        ...
        if state is not None:
            self.set_state_dict(resolve_state_dict(state))
            return
        self.rq_args = dict(r=r, buffersize=buffersize, seed=seed)
    ...
    def most_common_conditions(self, n):
        return sorted(self.call_stats.keys(), key=lambda c: -self.call_stats[c])[:n]
    ...
    def collected_quantiles(self, conditions, quantiles, old_style=False):
        ...
        for i, c in enumerate(conditions):
            if c in self.running_quantiles:
                result[i] = self.running_quantiles[c].quantiles(quantiles, old_style)
        return result
    ...
    def set_state_dict(self, dic):
        ...
        for k, v in dic.items():
            if "." in k:
                ...
                subdicts[p][s] = v
        self.running_quantiles = {
            c: RunningQuantile(state=subdicts[str(i)]) for i, c in enumerate(conditions)
        }
        if conditions:
    ...
    def set_state_dict(self, dic):
        ...
        for k, v in dic.items():
            if "." in k:
                ...
                subdicts[p][s] = v
        self.running_var = {
            c: RunningVariance(state=subdicts[str(i)]) for i, c in enumerate(conditions)
        }
...
class RunningCrossCovariance:
    ...
    def add(self, a, b):
        ...
        if self._mean is None:
            ...
            progress_addbmm(
                self.cmom2, centered[0][:, :, None], centered[1][:, None, :], sub_batch
            )
            return
        # Update a batch using Chan-style update for numerical stability.
        oldcount = self.count
        ...
        new_frac = float(batch_count) / self.count
        # Update the mean according to the batch deviation from the old mean.
        delta = [bm.sub_(m).mul_(new_frac) for bm, m in zip(batch_mean, self._mean)]
        ...
        self.cmom2.addmm_(
            alpha=new_frac * oldcount, mat1=delta[0][:, None], mat2=delta[1][None, :]
        )
        # Update the variance using the batch deviation
    ...
    def set_state_dict(self, dic):
        ...
        self.v_cmom2 = [torch.from_numpy(dic[k]) for k in ["cmom2_a", "cmom2_b"]]
...
class RunningCovariance:
    ...
    def add(self, a):
        ...
        if self._mean is None:
            ...
            progress_addbmm(
                self.cmom2, centered[:, :, None], centered[:, None, :], sub_batch
            )
            return
        # Update a batch using Chan-style update for numerical stability.
        oldcount = self.count
    ...
    def to_(self, device):
        self._mean, self.cmom2 = [m.to(device) for m in [self._mean, self.cmom2]]
