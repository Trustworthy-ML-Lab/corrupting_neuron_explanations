...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Net dissect utility", prog="python -m %s.sampler" % __package__
    )
    ...
    for i in sampler.dereference(range(args.size)):
        ...
        shutil.copy(
            os.path.join(args.indir, sourcefile), os.path.join(args.outdir, filename)
        )
...
def test():
    ...
    assert_almost_equal(
        coordinate_sample((26, 26), 10, range(101, 102)),
        [[[14, 0, 12, 11, 8, 13, 11, 20, 7, 20], [9, 22, 7, 11, 23, 18, 21, 15, 2, 5]]],
    )
    assert_almost_equal(
        coordinate_sample((13, 13), 10, range(101, 102)),
        [
            [
    ...
    assert_almost_equal(
        coordinate_sample((13, 13), 10, range(100, 102), flat=True),
        [
    ...
    assert_almost_equal(
        coordinate_sample((13, 13), 10, range(101, 103), flat=True),
        [
    ...
    assert_almost_equal(
        coordinate_sample((26, 26), 10, range(101, 102), flat=True),
    ...
    fss = FixedRandomSubsetSampler([(i, i % 3) for i in range(10)]).class_subset(
        class_filter=1
    )
    assert len(fss) == 3
