    ...
    def predict_single_class(self, tensor_images, classnum, downsample=1):
        for i, sgm in enumerate(self.component_segmenters):
            if i + 1 == len(self.offsets) or classnum < self.offsets[i + 1]:
                ...
                break
        return sgm.predict_single_class(tensor_images, classnum, downsample=downsample)
...
class UnifiedParsingSegmenter(BaseSegmenter):
    ...
    def __init__(self, segsizes=None, segdiv=None, all_parts=False):
        ...
        segmodel = load_unified_parsing_segmentation_model(segarch, segvocab, epoch)
        segmodel.cuda()
        ...
        partnumbers = {}
        partnames = []
        objectnumbers = {k: v for v, k in enumerate(segmodel.labeldata["object"])}
    ...
    def get_label_and_category_names(self, dataset=None):
        ...
        for suffix in suffixes:
            divided_labels.extend(
                [
                    ("%s-%s" % (label, suffix), "part")
                    for label in self.segmodel.labeldata["object"][1:]
                ]
            )
        # Create the whole list of labels
        labelcats = (
            [(label, "object") for label in self.segmodel.labeldata["object"]]
            + divided_labels
            + [(label, "material") for label in self.segmodel.labeldata["material"][1:]]
    ...
    def raw_seg_prediction(self, tensor_images, downsample=1):
        ...
        pred = {
            category: torch.zeros(
                len(tensor_images),
                len(self.segmodel.labeldata[category]),
                seg_shape[0],
                seg_shape[1],
            ).cuda()
        ...
        part_pred = {
            partobj_index: torch.zeros(
                len(tensor_images), len(partindex), seg_shape[0], seg_shape[1]
            ).cuda()
        ...
        for size in sizes:
            ...
            r_pred = self.segmodel(dict(img=resized), seg_size=seg_shape)
            for k in pred:
                pred[k] += r_pred[k]
            for k in part_pred:
    ...
    def segment_batch(self, tensor_images, downsample=1):
        ...
        are downsampled from the original image.
        """
        pred, part_pred = self.raw_seg_prediction(tensor_images, downsample=downsample)
    ...
    def predict_single_class(self, tensor_images, classnum, downsample=1):
        ...
        result = 0
        pred, part_pred = self.raw_seg_prediction(tensor_images, downsample=downsample)
        material_offset = (len(self.segmodel.labeldata["object"]) - 1) * self.divmult
        if (
            material_offset
            < classnum
            < material_offset + len(self.segmodel.labeldata["material"])
        ):
            return (
...
class SemanticSegmenter(BaseSegmenter):
    def __init__(
        ...
        assert (
            ...
            == list(c for c in segmodel.modules() if isinstance(c, torch.nn.Conv2d))[
                -1
            ].out_channels
        )
        segmodel.cuda()
        ...
        for catname, indexlist in self.category_indexes.items():
            for index, i in enumerate(indexlist):
                self.catindexmap[segmodel.meta.labels[i].name] = (catname, index)
        # After the softmax, each category is mapped to external indexes.
        self.category_map = {
            catname: torch.tensor(
                [
        ...
        if self.segdiv == "quad":
            ...
            for suffix in suffixes:
                divided_labels.extend(
                    [("%s-%s" % (label, suffix), cat) for label, cat in self.labels[1:]]
                )
    ...
    def raw_segment_batch(self, tensor_images, downsample=1):
        ...
        for i, cat in enumerate(self.category_rules):
            ...
            segs[:, i] = translated
        if self.segdiv == "quad":
            segs = self.expand_segment_quad(
                segs, self.num_underlying_classes, self.segdiv
            )
        return segs, pred
    ...
    def raw_seg_prediction(self, tensor_images, downsample=1):
        ...
        for size in sizes:
            if size == tensor_images.shape[2:]:
                ...
                resized = torch.nn.AdaptiveAvgPool2d(size)(tensor_images)
            raw_pred = self.segmodel(dict(img_data=resized), segSize=seg_shape)
    ...
    def predict_single_class(self, tensor_images, classnum, downsample=1):
        ...
        Does not work on subdivided labels.
        """
        seg, pred = self.raw_segment_batch(tensor_images, downsample=downsample)
...
def load_unified_parsing_segmentation_model(segmodel_arch, segvocab, epoch):
    ...
    nr_classes = {k: len(labeldata[k]) for k in ["object", "scene", "material"]}
    ...
    segmodel = upsegmodel.SegmentationModule(seg_encoder, seg_decoder, labeldata)
...
def load_segmentation_model(modeldir, segmodel_arch, segvocab, epoch=None):
    ...
    if epoch is None:
        choices = [
            os.path.basename(n)[14:-4]
            for n in glob.glob(os.path.join(segmodel_dir, "encoder_epoch_*.pth"))
        ]
...
def test_main():
    ...
    tensor_im = (
        torch.from_numpy(numpy.asarray(testim)).permute(2, 0, 1).float() / 255 * 2 - 1
    )[None, :, :, :].cuda()
    ...
    for label in bc.nonzero()[:, 0]:
        if label.item():
            ...
            print(
                "%s (%s, #%d): %d pixels, pred %.2g inside %.2g outside"
                % (
                    labels[label.item()]
                    + (label.item(), bc[label].item(), inside_pred, outside_pred)
                )
            )
