...
def tally_topk(
    compute, dataset, sample_size=None, batch_size=10, k=100, cachefile=None, **kwargs
):
    """
...
def expand_topk(topk, channels):
    ...
    rtk.top_data = rtk.top_data.expand((channels, rtk.top_data.size(1))).clone()
    rtk.top_index = rtk.top_index.expand((channels, rtk.top_index.size(1))).clone()
    return rtk


def gather_topk(compute, dataset, topk, k=None, cachefile=None, **kwargs):
    """
    ...
    for batchnum, batch in enumerate(pbar(loader)):
        ...
        batch.insert(
            0,
            [
                needed_images[needed_sample[index + batchnum * loader.batch_size]]
...
def tally_conditional_topk(
    compute, dataset, k=100, batch_size=50, sample_size=None, cachefile=None, **kwargs
):
    """
...
def tally_quantile(
    compute, dataset, sample_size=None, batch_size=10, r=4096, cachefile=None, **kwargs
):
    """
...
def tally_extremek_and_quantile(
    compute,
    dataset,
    sample_size=None,
    batch_size=10,
...
def tally_conditional_quantile(
    compute,
    dataset,
    sample_size=None,
    batch_size=1,
    ...
    **kwargs,
    ...
    for i, batch in enumerate(pbar(loader)):
        ...
        if i and not i & (i - 1):  # if i is a power of 2:
            ...
            cq.to_("cpu", [k for k in cq.keys() if k not in common_conditions])
    # At the end, move all to the CPU
    cq.to_("cpu")
...
def conditional_samples(activations, segments):
    ...
    def sample_generator():
        ...
        for condition in conditions:
            ...
            yield (condition.item(), activations_by_channel[mask].view(-1, channels))

    return sample_generator()


def tally_mean(
    compute, dataset, sample_size=None, batch_size=10, cachefile=None, **kwargs
):
    """
...
def tally_conditional_mean(
    compute, dataset, sample_size=None, batch_size=1, cachefile=None, **kwargs
):
    """
...
def tally_all_intersection_and_union(
    compute, dataset, sample_size=None, batch_size=10, cachefile=None, **kwargs
):
    """
...
def batch_bincount(data, num_labels):
    ...
    strided = (
        data
        + torch.arange(len(data), dtype=data.dtype, device=data.device)[:, None]
        * num_labels
    )
...
def iou_from_conditional_quantile(condq, cutoff=0.95, min_batches=2):
    ...
    """
    return intersection_from_conditional_quantile(
        condq, statistic=intersection_over_union, cutoff=cutoff, min_batches=min_batches
    )
...
def mi_from_conditional_quantile(condq, cutoff=0.95, min_batches=2):
    ...
    """
    return intersection_from_conditional_quantile(
        condq, statistic=mutual_information, cutoff=cutoff, min_batches=min_batches
    )
...
def intersection_from_conditional_quantile(
    ...
    for c in prog(sorted(condq.keys())):
        ...
        arr = torch.stack([isect, pred - isect, truth - isect, 1 - union]).view(
            (2, 2) + isect.shape
        )
...
def make_loader(
    ...
    **kwargs,
    ...
    logger.info(f"Creating dataloader with transform {transform_dataloader}")

    class TransformDataloader(torch.utils.data.DataLoader):
        """
        Modify the dataloader to apply a transform to each batch.
        """

        def __init__(self, transform_dataloader, **kwargs):
            self.transform_dataloader = transform_dataloader
            super(TransformDataloader, self).__init__(**kwargs)

        def transform_iter(self, iter):
            for batch in iter:
                batch = list(transforms.map_location(batch, device))
                if transform_dataloader is not None:
                    batch = transform_dataloader(*batch)
                yield batch

        def __iter__(self):
            return self.transform_iter(super(TransformDataloader, self).__iter__())
...
def torch_from_numpy_dict(state):
    return {
        k: torch.from_numpy(v) if len(v.shape) > 0 else v.item()
    ...
    try:
        ...
        for a, v in args.items():
            if a not in dat or dat[a] != v:
                pbar.print("%s %s changed from %s to %s" % (cachefile, a, dat[a], v))
                return None
    except:
        return None
    else:
