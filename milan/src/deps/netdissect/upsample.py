...
def upsampler(
    ...
    grid = upsample_grid(
        data_shape, target_shape, image_size, scale_offset, dtype, device
    )
    batch_grid = grid
    ...
    def upsample_func(data, mode="bilinear", padding_mode="zeros"):
        ...
        try:
            ...
            return torch.nn.functional.grid_sample(
                data, batch_grid, mode=mode, padding_mode=padding_mode
...
def sequence_data_size(modulelist, input_size):
    ...
    """
    return tuple(
        convconfig_data_size(d, s) for d, s in zip(convconfigs(modulelist), input_size)
    )
...
def convconfig_data_size(convconfigs, data_size):
    ...
    for kernel, dilation, stride, padding in convconfigs:
        data_size = (
            1 + (data_size + 2 * padding - dilation * (kernel - 1) - 1) // stride
        )
    return data_size
...
def convconfigs(modulelist):
    ...
    for module in modulelist:
        ...
        settings = tuple((s if isinstance(s, tuple) else (s, s)) for s in settings)
...
def upsample_grid(
    ...
    if scale_offset is None:
        scale = tuple(float(ts) / ds for ts, ds in zip(target_shape, data_shape))
    ...
    ty, tx = (
        (
            (torch.arange(ts, dtype=dtype, device=device) - o)
            * (2 / (s * max(1, (ss - 1))))
            - 1
        )
