...
def standard_y_sample(size, num_classes, seed=1, device=None):
    ...
    # without affecting global state
    rng = numpy.random.RandomState(seed)
    result = torch.from_numpy(rng.randint(num_classes, size=size)).long()
    if device is not None:
        result = result.to(device)
    return result
...
def training_loader(z_generator, batch_size, loader_size=10000):
    ...
    g_epoch = 1
    while True:
        z_data = z_dataset_for_model(z_generator, size=loader_size, seed=g_epoch + 1)
...
def testing_loader(z_generator, batch_size, test_size=1000):
    ...
    """
    z_data = z_dataset_for_model(z_generator, size=test_size, seed=1)
    dataloader = torch.utils.data.DataLoader(
        z_data, shuffle=False, batch_size=batch_size, num_workers=10, pin_memory=True
    )
    return dataloader
