...
import src.deps.pretorched.layers as layers
        ...
        **kwargs
        ...
        for index in range(len(self.arch["out_channels"])):
            ...
            if self.arch["attention"][self.arch["resolution"][index]]:
                if self.verbose:
                    print(
                        "Adding attention layer in D at resolution {}".format(
                            self.arch["resolution"][index]
                        )
                    )
                self.blocks[-1] += [
...
model_urls = {
    ...
    "places365-challenge": {
        256: {
            "D": os.path.join(
                root_url, "biggan_deep256_D_places365-challenge-c4fb8bfe.pth"
            ),
            "G": os.path.join(
                root_url, "biggan_deep256_G_places365-challenge-1d2bae3c.pth"
            ),
            "G_ema": os.path.join(
                root_url, "biggan_deep256_G_ema_places365-challenge-c3a49c8a.pth"
            ),
            "state_dict": os.path.join(
                root_url, "biggan_deep256_state_dict_places365-challenge-dadfc659.pth"
            ),
        }
    },
}
