...
root_url = "http://pretorched-x.csail.mit.edu/gans/ProGAN"


def proggan(pretrained="lsun_bedroom", resolution=256):
    # Automatically download and cache progressive GAN model
    # (From Karras, converted from Tensorflow to Pytorch.)
    if pretrained is not None and resolution is None:
        resolution = {"celeba_hq": 1024}.get(pretrained, 256)
    weights_filename = dict(
        lsun_bedroom="proggan_bedroom-d8a89ff1.pth",
        lsun_church="proggan_churchoutdoor-7e701dd5.pth",
        lsun_conferenceroom="proggan_conferenceroom-21e85882.pth",
        lsun_diningroom="proggan_diningroom-3aa0ab80.pth",
        lsun_kitchen="proggan_kitchen-67f1e16c.pth",
        lsun_livingroom="proggan_livingroom-5ef336dd.pth",
        lsun_restaurant="proggan_restaurant-b8578299.pth",
        celeba_hq="proggan_celebhq-620d161c.pth",
    )[pretrained]
    url = os.path.join(root_url, weights_filename)
    try:
...
class ProgressiveGenerator(nn.Sequential):
    ...
    def __init__(
        self, resolution=None, sizes=None, modify_sequence=None, output_tanh=True
    ):
        """
        ...
        add_d(
            OutputConvBlock(sizes[-1], tanh=output_tanh),
...
class NormConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding):
        ...
        self.wscale = WScaleLayer(
            out_channels, in_channels, gain=numpy.sqrt(2) / kernel_size
        )
...
def state_dict_from_tf_parameters(parameters):
    ...
    for i in itertools.count():
        ...
        if i == 0:
            ...
            weight = (
                weight.view(weight.shape[0], weight.shape[1] // 16, 4, 4)
                .permute(1, 0, 2, 3)
                .flip(2, 3)
            )
            sizes.append(weight.shape[0])
...
def state_dict_from_old_pt_dict(params):
    ...
    for i in itertools.count():
        ...
        if i % 2 == 0:
            sizes.append(weight.shape[1])
        result["%s.conv.weight" % (pt_layername)] = weight
        result["%s.wscale.b" % (pt_layername)] = params["%s.wscale.b" % (old_layername)]
