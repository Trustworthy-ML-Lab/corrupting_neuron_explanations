...
class ScaledParamModule(nn.Module):
    ...
    def scale_weight(
        self, gain=np.sqrt(2), use_wscale=True, lrmul=1, new_name="_weight"
    ):
        weight = self.weight
    ...
    def add_scale_hook(self, name, new_name, coef):
        ...
        self.register_forward_pre_hook(
            lambda m, inp: setattr(m, name, getattr(m, new_name) * coef)
        )
...
class ScaledParamLinear(nn.Linear, ScaledParamModule):
    ...
    def extra_repr(self):
        return "in_features={}, out_features={}, bias={}".format(
            self.in_features,
            self.out_features,
            self._bias is not None,  # use the _real param
        )
    ...
    def forward(self, x):
        if self.do_fuse:
            ...
            return F.conv_transpose2d(
                x, w, self.bias, stride=(2, 2), padding=self.padding
            )
        else:
...
class Blur2d(nn.Module):
    def __init__(
        self, kernel=[1, 2, 1], padding=1, normalize=True, flip=False, stride=1
    ):
        super().__init__()
...
class MappingG(nn.Module):
    def __init__(
        ...
        for i in range(n_layers):
            self.fcs.append(
                ScaledParamLinear(
                    dim, dim_latent if i < (n_layers - 1) else w_dim, **scale_param_opt
                ),
            )
            dim = dim_latent
...
class SynthesisG(nn.Module):
    class AddNoise(ScaledParamModule):
        ...
        def forward(self, x, noise=None):
            if noise is None:
                noise = torch.randn(
                    x.size(0), 1, self.res, self.res, device=x.device, dtype=x.dtype
                )
    ...
    class Block(nn.Module):
        def __init__(
        ...
        ):  # skip_first_layer skips the upsample & first conv, used for first block
            super().__init__()
            self.skip_first_layer = skip_first_layer
            self.act = nonlinearity

            scale_param_opt = dict(
                gain=self.act.gain, lrmul=lrmul, use_wscale=use_wscale
            )
            ...
            if not self.skip_first_layer:
                self.upconv1 = UpConv2d(
                    in_res, in_nc, out_nc, 3, padding=1, bias=False, **scale_param_opt
                )
            ...
            self.conv2 = ScaledParamConv2d(
                out_nc, out_nc, 3, padding=1, bias=False, **scale_param_opt
            )
    ...
    def __init__(
        ...
        self.const = nn.Parameter(
            torch.ones(1, get_out_nc(1), 4, 4, requires_grad=True)
        )
        ...
        for in_log_res in range(1, log_image_out_res):
            ...
            to_rgb = ScaledParamConv2d(
                out_nc, image_out_nc, 1, gain=1, use_wscale=use_wscale, lrmul=lrmul
            )
            out_log_res = in_log_res + 1
            self.add_module(
                "{res}x{res}_to_rgb_lod{lod}".format(
                    res=out_res, lod=(log_image_out_res - out_log_res)
                ),
                to_rgb,
            )
    ...
    def rgb_convs(self):
        ...
        for in_log_res in range(1, log_out_res):
            out_log_res = in_log_res + 1
            name = "{res}x{res}_to_rgb_lod{lod}".format(
                res=out_res, lod=(log_out_res - out_log_res)
            )
            module = children_dict[name]
    ...
    def forward(self, ws, lod=0, alpha=1, noises=None):
        ...
        for i, b in enumerate(blocks):
            ...
            x = b(
                x, ws[block_extra_inp_indices], noises=noises[block_extra_inp_indices]
            )
            if i == stop_after - 1:
...
class G(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.synthesis = SynthesisG(
            w_dim, out_nc, out_res, nonlinearity=nonlinearity, use_wscale=use_wscale
        )
    ...
    def forward(
        ...
        if self.training:
            ...
            if (
                style_mixing_prob > 0
                and torch.rand((), device="cpu").item() < style_mixing_prob
            ):
                w2 = self.mapping(torch.randn_like(z), y)
                cutoff = int(
                    torch.randint(
                        low=1, high=forward_num_layers, size=(), device="cpu"
                    ).item()
                )
            ...
            if truncation_cutoff > 0 and truncation_psi != 1:
                ...
                if all(_w is w for _w in ws):
                    ...
                    ws = [truncate_w for _ in range(truncation_cutoff)] + ws[
                        : (forward_num_layers - truncation_cutoff)
                    ]
                else:
    ...
    def convert_tf_weights_to_state_dict(self, tf_net, device=torch.device("cpu")):
        ...
        def raise_unexpected(tf_k, v):
            raise RuntimeError(
                "Unexpected key '{}' with shape: {}".format(tf_k, v.size())
            )
        ...
        def sub_synthesis(k, tf_k, v):
            ...
            if replace_tail_if_match(
                ".Conv0_up.weight", ".upconv1._weight"
            ):  # noqa: E241, E202
                return k, transform_conv_w(v)
            if replace_tail_if_match(
                ".Conv0_up.Noise.weight", ".noise1.weight"
            ):  # noqa: E241, E202
                return k, v.view(-1, 1, 1)
            if replace_tail_if_match(
                ".Conv0_up.bias", ".noise1._bias"
            ):  # noqa: E241, E202
                return k, v.view(-1, 1, 1)
            if replace_tail_if_match(
                ".Conv0_up.StyleMod.weight", ".style1.fc._weight"
            ):  # noqa: E241, E202
                return k, transform_fc_w(v)
            if replace_tail_if_match(
                ".Conv0_up.StyleMod.bias", ".style1.fc._bias"
            ):  # noqa: E241, E202
                return k, v
            ...
            if replace_tail_if_match(
                ".Conv1.weight", ".conv2._weight"
            ):  # noqa: E241, E202
                return k, transform_conv_w(v)
            if replace_tail_if_match(
                ".Conv1.Noise.weight", ".noise2.weight"
            ):  # noqa: E241, E202
                return k, v.view(-1, 1, 1)
            if replace_tail_if_match(
                ".Conv1.bias", ".noise2._bias"
            ):  # noqa: E241, E202
                return k, v.view(-1, 1, 1)
            if replace_tail_if_match(
                ".Conv1.StyleMod.weight", ".style2.fc._weight"
            ):  # noqa: E241, E202
                return k, transform_fc_w(v)
            if replace_tail_if_match(
                ".Conv1.StyleMod.bias", ".style2.fc._bias"
            ):  # noqa: E241, E202
                return k, v
...
class D(nn.Module):
    ...
    class ConvBlock(nn.Module):
        def __init__(
            ...
            if not self.last_layer:
                self.blur = Blur2d(
                    kernel=[1, 2, 1], normalize=True, stride=1, padding=1
                )
                self.pool = Blur2d(
                    kernel=[0.5, 0.5], normalize=False, stride=2, padding=0
                )
                self.conv1 = ScaledParamConv2d(
                    in_nc, in_nc, 3, padding=1, bias=True, **scale_param_opt
                )
                self.conv2 = ScaledParamConv2d(
                    in_nc, out_nc, 3, padding=1, bias=True, **scale_param_opt
                )
            else:
                self.minibatch_stddev = D.MinibatchStddev()
                self.conv1 = ScaledParamConv2d(
                    in_nc + 1, in_nc, 3, padding=1, bias=True, **scale_param_opt
                )
                self.conv2 = ScaledParamLinear(
                    in_nc * 16, out_nc, bias=True, **scale_param_opt
                )
    ...
    def __init__(
        ...
        **kwargs
        ...
        for in_log_res in reversed(range(1, log_out_res)):
            ...
            from_rgb = ScaledParamConv2d(
                image_in_nc,
                in_nc,
                kernel_size=1,
                gain=nonlinearity.gain,
                bias=True,
                use_wscale=use_wscale,
                lrmul=lrmul,
            )
            ...
            b = D.ConvBlock(
                in_nc,
                out_nc,
                last_layer=(in_log_res == log_out_res - 1),
                nonlinearity=nonlinearity,
                use_wscale=use_wscale,
                lrmul=lrmul,
            )
            ...
            out_log_res = in_log_res + 1
            self.add_module(
                "{res}x{res}_from_rgb_lod{lod}".format(
                    res=out_res, lod=(out_log_res - 2)
                ),
                from_rgb,
            )
        ...
        if self.use_class_labels:
            self.fc = ScaledParamLinear(
                min(get_out_nc(log_out_res - 1), nc_max),
                nlabels,
                gain=1.0,
                bias=True,
                use_wscale=True,
    ...
    def rgb_convs(self):
        ...
        for in_log_res in reversed(range(1, log_out_res)):
            out_log_res = in_log_res + 1
            name = "{res}x{res}_from_rgb_lod{lod}".format(
                res=out_res, lod=(out_log_res - 2)
            )
            module = children_dict[name]
            rgb_convs.append(module)
    ...
    def convert_tf_weights_to_state_dict(self, tf_net, device=torch.device("cpu")):
        ...
        def sub_synthesis(k, tf_k, v):
            ...
            if replace_tail_if_match(
                ".Conv1_down.weight", ".conv2._weight"
            ):  # noqa: E241, E202
                return k, transform_conv_w(v)
            if replace_tail_if_match(
                ".Conv1_down.bias", ".conv2._bias"
            ):  # noqa: E241, E202
                return k, v.view(-1)
