...
from src.deps.pretorched.utils import cache

from .base import History, LoggerBase, rank_zero_only
...
class TensorBoardLogger(LoggerBase):
    ...
    def __init__(
        self, logs_root="logs", name="default", version=None, rank=0, **kwargs
    ):
        super().__init__()
...
if __name__ == "__main__":
    ...
    for epoch in range(10):
        for n_iter in range(100):
            itr += 1
            writer.add_scalars(
                "Loss", {"train": np.random.random(), "val": np.random.random()}, itr
            )
