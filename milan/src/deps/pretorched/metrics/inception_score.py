...
class WrapInception(nn.Module):
    """Module that wraps the inception network.

    Enables use with dataparallel and returns pool features and logits.
...
def sqrt_newton_schulz(A, numIters, dtype=None):
    """Matrix sqrt using newton shculz method.

    From Tsung-Yu Lin, and Subhransu Maji:
...
def load_inception_net(gpu=None, distributed=None, device="cuda"):
    ...
    Args:
        config (dict): with format:
            distributed (bool): Whether to use DistributedDataParallel.
            gpu (int or None): Which GPU to use.
    ...
    if distributed:
        if gpu is not None:
            ...
            inception_model = torch.nn.parallel.DistributedDataParallel(inception_model)
    ...
    def get_inception_metrics(
        ...
        if use_torch:
            ...
            FID = numpy_calculate_frechet_distance(
                mu.cpu().numpy(), sigma.cpu().numpy(), data_mu, data_sigma
            )
...
def calculate_inception_moments(dataloader, root, name, gpu=None, device="cuda"):
    ...
    print("Training data from has IS of {:5.5f} +/- {:5.5f}".format(IS_mean, IS_std))
