...
from src.deps.pretorched.runners import config as cfg
from src.deps.pretorched.runners import core
...
parser.add_argument(
    "--data_root", metavar="DIR", default="/data/vision/oliva/scratch/datasets"
)
parser.add_argument(
    "-a",
    "--arch",
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    try:
        train_loader.dataset.transforms.transforms[-1] = transforms.Normalize(
            mean=mean, std=std
        )
        val_loader.dataset.transforms.transforms[-1] = transforms.Normalize(
            mean=mean, std=std
        )
    except Exception:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (
            ...
            save_checkpoint(
                ...
                is_best,
                filename=f"{args.dataset}_{args.arch}_checkpoint.pth.tar",
            )
