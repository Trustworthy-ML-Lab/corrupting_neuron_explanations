...
import src.deps.pretorched.models as models
...
IMAGE_DATASETS = ["ImageNet", "Places365", "Hybrid1365"]
...
def get_root_dirs(name, dataset_type="ImageHDF5", resolution=128, data_root=DATA_ROOT):
    root_dirs = {
        ...
        "M3": {
            "VideoRecordZipDataset": defaultdict(
                lambda: os.path.join(data_root, "Moments/videos")
            )
        },
    }
...
def get_metadata(
    ...
    root_dirs = {
        "DFDC": {
            ...
            "DeepfakeFaceVideo": defaultdict(
                lambda: os.path.join(
                    data_root, "DeepfakeDetection/facenet_smooth_frames"
                ),
                {},
            ),
        ...
        "Moments": {
            "VideoRecordDataset": defaultdict(
                lambda: os.path.join(data_root, "Moments/videos"), {}
            ),
            "VideoRecordZipDataset": defaultdict(
                lambda: os.path.join(data_root, "Moments/videos"), {}
        ...
        "Kinetics": {
            "VideoRecordDataset": defaultdict(
                lambda: os.path.join(data_root, "Kinetics/videos")
            ),
            "VideoRecordZipDataset": defaultdict(
                lambda: os.path.join(data_root, "Kinetics/videos")
            ),
    ...
    metafiles = {
        "Moments": {
            "RecordSet": defaultdict(
                lambda: os.path.join(data_root, "Moments", fname), {}
            ),
        },
        "M3": {
        ...
        "Kinetics": {
            "RecordSet": defaultdict(
                lambda: os.path.join(data_root, "Kinetics", fname), {}
            ),
        },
        "MultiMoments": {
            "RecordSet": defaultdict(
            ...
            "MultiLabelRecordSet": defaultdict(
                lambda: os.path.join(data_root, "Moments", fname), {}
            ),
        },
    }
