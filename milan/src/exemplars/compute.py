...
from src.deps.netdissect import imgsave, nethook, pbar, renormalize, runningstats, tally
...
def compute(
    compute_topk_and_quantile: Callable[..., TensorPair],
    ...
    transform_dataloader: Optional[Any] = None,
    device=None,
) -> ActivationStats:
    ...
    if quantile <= 0 or quantile >= 1:
        raise ValueError(
            "must have quantile in range (0, 1), " f"got quantile={quantile}"
        )
    if image_size is None and not hasattr(dataset, "transform"):
        raise ValueError(
            "dataset has no `transform` property so " "image_size= must be set"
        )
    ...
    if save_results:
        ...
        numpy.save(f"{results_dir}/images.npy", images.cpu())
        numpy.save(f"{results_dir}/masks.npy", masks.cpu())
        ...
        for metadata, name, fmt in (
            (activations, "activations", "%.5e"),
            (ids, "ids", "%i"),
        ):
    ...
    # save tiled outputs
    if save_viz:
        # Save tiles masked and unmasked images
        logger.info("Saving tiles of masked images")
        masked = [
            [mi.cpu().numpy() for mi in mis] for mis in masked.permute(0, 1, 3, 4, 2)
        ]
        imgsave.save_image_tile(
            masked, f"{viz_dir}/unit_%d_masked.png", sourcefile=masks_cache_file
        )

        logger.info("Saving tiles of unmasked images")
        individual = [
            [mi.cpu().numpy() for mi in mis] for mis in images.permute(0, 1, 3, 4, 2)
        ]
        imgsave.save_image_tile(
            individual, f"{viz_dir}/unit_%d_original.png", sourcefile=masks_cache_file
        )
    ...
    with nethook.InstrumentedModel(model) as instr:
        ...
        def compute_topk_and_quantile(*inputs: Any) -> TensorPair:
            ...
            if len(hiddens.shape) == 2:
                hiddens = hiddens.unsqueeze(2).unsqueeze(3)
                # print(hiddens.shape)
...
def run(
    ...
    **kwargs: Any,
    ...
    results_dir = resolve(results_dir) / "output"
    viz_dir = resolve(viz_dir)
