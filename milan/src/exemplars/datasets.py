...
from src.dataloader.imagenet_kaggle import ImageNetKaggle
...
class TensorDatasetOnDisk(torch.utils.data.TensorDataset):
    ...
    def __init__(self, root: PathLike, **kwargs: Any):
        ...
        loaded = sorted(loaded, key=lambda tensor: not tensor.dtype.is_floating_point)
...
def default_dataset_configs(
    **others: hubs.DatasetConfig,  # Your overrides!
    ...
    configs = {
        KEYS.IMAGENET: hubs.DatasetConfig(
            ImageNetKaggle,
            split="val",
        ...
        KEYS.PLACES365: hubs.DatasetConfig(
        ...
        KEYS.IMAGENET_SPURIOUS_TEXT: hubs.DatasetConfig(
            torchvision.datasets.ImageFolder,
            transform=torchvision.transforms.Compose(
                [
        ...
        KEYS.IMAGENET_SPURIOUS_COLOR: hubs.DatasetConfig(
            torchvision.datasets.ImageFolder,
            transform=torchvision.transforms.Compose(
                [
                    torchvision.transforms.Resize((224, 224)),
        ...
        KEYS.BIGGAN_ZS_IMAGENET: hubs.DatasetConfig(
            TensorDatasetOnDisk,
            url=f"{HOST}/{KEYS.BIGGAN_ZS_IMAGENET}.zip",
        ),
        KEYS.BIGGAN_ZS_PLACES365: hubs.DatasetConfig(
            TensorDatasetOnDisk,
            url=f"{HOST}/{KEYS.BIGGAN_ZS_PLACES365}.zip",
        ),
        KEYS.CIFAR10: hubs.DatasetConfig(
...
def load(
    name: str, configs: Optional[Mapping[str, hubs.DatasetConfig]] = None, **kwargs: Any
