...
from cifar10.cifar10_models.vgg import vgg16_bn
...
LAYERS.SQUEEZENET1_0 = (f"features.{index}" for index in (0, 3, 4, 5, 7, 8, 9, 10, 12))
...
LAYERS.VGG16_BN = tuple(f"features.{index}" for index in (3, 10, 20, 30, 40))
...
class GenerativeModelExemplarsConfig(ModelExemplarsConfig):
    ...
    def __post_init__(self) -> None:
        ...
        if self.dataset is None:
            raise ValueError(
                "GenerativeModelExemplarsConfig requires " "dataset to be set"
            )
...
def default_model_configs(**others: ModelConfig) -> Mapping[str, ModelConfig]:
    ...
    configs = {
        KEYS.ALEXNET_IMAGENET: ModelConfig(
            models.alexnet_seq,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.ALEXNET,
        ),
        KEYS.ALEXNET_IMAGENET_BLURRED: ModelConfig(
            models.alexnet,
            load_weights=True,
            layers=(
                "features.0",
        ...
        KEYS.BIGGAN_IMAGENET: ModelConfig(
            biggan.SeqBigGAN,
            pretrained="imagenet",
            load_weights=False,
        ...
        KEYS.BIGGAN_PLACES365: ModelConfig(
            biggan.SeqBigGAN,
        ...
        KEYS.DENSENET121_IMAGENET: ModelConfig(
            models.densenet121,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.DENSENET121,
        ),
        KEYS.DENSENET121_IMAGENET_BLURRED: ModelConfig(
            models.densenet121, load_weights=True, layers=LAYERS.DENSENET121
        ),
        KEYS.DENSENET201_IMAGENET: ModelConfig(
            models.densenet201,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.DENSENET201,
        ),
        KEYS.DENSENET201_IMAGENET_BLURRED: ModelConfig(
            models.densenet201, load_weights=True, layers=LAYERS.DENSENET201
        ),
        KEYS.DINO_VITS8_IMAGENET: ModelConfig(
            torch.hub.load,
            repo_or_dir="facebookresearch/dino:main",
            model=KEYS.DINO_VITS8,
            layers=LAYERS.DINO_VITS8,
            exemplars=DiscriminativeModelExemplarsConfig(
                transform_hiddens=transforms.spatialize_vit_mlp, batch_size=32
            ),
            load_weights=False,
        ),
        KEYS.MOBILENET_V2_IMAGENET: ModelConfig(
            models.mobilenet_v2,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.MOBILENET_V2,
        ),
        KEYS.MOBILENET_V2_IMAGENET_BLURRED: ModelConfig(
            models.mobilenet_v2, load_weights=True, layers=LAYERS.MOBILENET_V2
        ),
        KEYS.RESNET18_IMAGENET: ModelConfig(
        ...
        KEYS.RESNET18_IMAGENET_BLURRED: ModelConfig(
            models.resnet18, load_weights=True, layers=LAYERS.RESNET18
        ),
        KEYS.RESNET18_PLACES365: ModelConfig(
            models.resnet18,
            load_weights=True,
            layers=LAYERS.RESNET18,
            # get rid of module.
            transform_weights=lambda weights: {
                key[7:]: weights["state_dict"][key] for key in weights["state_dict"]
            },
            url=f"{HOST}/resnet18_places365-2f475921.pth",
            num_classes=365,
        ),
        KEYS.RESNET34_IMAGENET: ModelConfig(
            models.resnet34,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.RESNET34,
        ),
        KEYS.RESNET34_IMAGENET_BLURRED: ModelConfig(
            models.resnet34, load_weights=True, layers=LAYERS.RESNET34
        ),
        KEYS.RESNET50_IMAGENET: ModelConfig(
            models.resnet50,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.RESNET50,
        ),
        KEYS.RESNET50_ROBUST_4x_IMAGENET: ModelConfig(
            models.resnet50,
            pretrained=False,
            load_weights=True,
            requires_path=True,
            state_dict="zoo/imagenet_model_weights_4px.pth.tar",
            state_dict_key="state_dict",
            state_dict_data_parallel=True,
            layers=LAYERS.RESNET50,
        ),
        KEYS.RESNET50_IMAGENET_BLURRED: ModelConfig(
            models.resnet50, load_weights=True, layers=LAYERS.RESNET50
        ),
        KEYS.RESNET101_IMAGENET: ModelConfig(
            models.resnet101,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.RESNET101,
        ),
        KEYS.RESNET101_IMAGENET_BLURRED: ModelConfig(
            models.resnet101, load_weights=True, layers=LAYERS.RESNET101
        ),
        KEYS.RESNET152_IMAGENET: ModelConfig(
            models.resnet152_seq,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.RESNET152,
        ),
        KEYS.RESNET152_IMAGENET_BLURRED: ModelConfig(
            models.resnet152, load_weights=True, layers=LAYERS.RESNET152
        ),
        KEYS.RESNET152_PLACES365: ModelConfig(
            resnet152.OldResNet152,
            url=f"{HOST}/resnet152_places365-f928166e5c.pth",
            layers=(0, 4, 5, 6, 7),
        ),
        KEYS.SHUFFLENET_V2_X1_0_IMAGENET: ModelConfig(
            models.shufflenet_v2_x1_0,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.SHUFFLENET_V2_X1_0,
        ),
        KEYS.SHUFFLENET_V2_X1_0_IMAGENET_BLURRED: ModelConfig(
            models.shufflenet_v2_x1_0,
            load_weights=True,
            layers=LAYERS.SHUFFLENET_V2_X1_0,
        ),
        KEYS.SQUEEZENET1_0_IMAGENET: ModelConfig(
            models.squeezenet1_0,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.SQUEEZENET1_0,
        ),
        KEYS.SQUEEZENET1_0_IMAGENET_BLURRED: ModelConfig(
            models.squeezenet1_0, load_weights=True, layers=LAYERS.SQUEEZENET1_0
        ),
        KEYS.VGG11_IMAGENET: ModelConfig(
            models.vgg11,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.VGG11,
        ),
        KEYS.VGG11_IMAGENET_BLURRED: ModelConfig(
            models.vgg11,
            load_weights=True,
            layers=LAYERS.VGG11,
        ),
        KEYS.VGG13_IMAGENET: ModelConfig(
            models.vgg13,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.VGG13,
        ),
        KEYS.VGG13_IMAGENET_BLURRED: ModelConfig(
            models.vgg13,
            load_weights=True,
            layers=LAYERS.VGG13,
        ),
        KEYS.VGG16_IMAGENET: ModelConfig(
            models.vgg16,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.VGG16,
        ),
        KEYS.VGG16_IMAGENET_BLURRED: ModelConfig(
            models.vgg16,
            load_weights=True,
            layers=LAYERS.VGG16,
        ),
        KEYS.VGG19_IMAGENET: ModelConfig(
            models.vgg19,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.VGG19,
        ),
        KEYS.VGG19_IMAGENET_BLURRED: ModelConfig(
            models.vgg19,
            load_weights=True,
            layers=LAYERS.VGG19,
        ),
        KEYS.VGG16_CIFAR10: ModelConfig(
            vgg16_bn,
            pretrained=True,
            load_weights=False,
            layers=LAYERS.VGG16_BN,
        ),
    }
...
def load(
    name: str, configs: Optional[Mapping[str, ModelConfig]] = None, **kwargs: Any
) -> Model:
