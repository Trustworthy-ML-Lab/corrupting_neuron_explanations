...
class Attention(serialize.SerializableModule):
    ...
    def __init__(
        self, query_size: int, key_size: int, hidden_size: Optional[int] = None
    ):
        ...
        self.output = nn.Sequential(nn.Linear(self.hidden_size, 1), nn.Softmax(dim=1))
...
class AllenNLPDecoderState(dict):
    ...
    def __init__(self, features: torch.Tensor, state: DecoderState, **kwargs: Any):
...
class Decoder(serialize.SerializableModule):
    ...
    def __init__(
        ...
        if lm is not None:
            ...
            if my_vocab != lm_vocab:
                raise ValueError(
                    "lm and decoder have different vocabs;"
        ...
        self.init_h = nn.Sequential(
            nn.Linear(self.feature_size, hidden_size), nn.Tanh()
        )
        ...
        self.attend = Attention(
            hidden_size, self.feature_size, hidden_size=attention_hidden_size
        )
        ...
        self.lstm = nn.LSTMCell(embedding_size + self.feature_size, hidden_size)
    ...
    def forward(
        ...
        if isinstance(strategy, torch.Tensor):
            ...
            if strategy.shape[-1] != length:
                raise ValueError(
                    f"strategy must have length {length}, " f"got {strategy.shape[-1]}"
                )
        ...
        if strategy not in {STRATEGY_BEAM, STRATEGY_RERANK}:
            ...
            predictions = features.new_zeros(batch_size, length, self.vocab_size)
            attentions = features.new_zeros(batch_size, length, features.shape[1])
            for time in range(length):
                outputs = self.step(features, currents, state, temperature=temperature)
        ...
        # Otherwise, if we're doing beam search, defer to allennlp.
            ...
            runner = beam_search.BeamSearch(
                self.indexer.stop_index, max_steps=length, beam_size=beam_size
            )

            def step(
                tokens: torch.Tensor, state: Dict[str, torch.Tensor]
            ) -> AllenNLPDecoderStep:
                """Take a decoding step."""
                assert isinstance(state, AllenNLPDecoderState)
                outputs = self.step(
                    state.features, tokens, state.state, temperature=temperature
                )
            ...
            beam_captions = tuple(map(self.indexer.reconstruct, tokens.tolist()))
            beam_scores = scores
            beam_tokens = tokens
            ...
            if strategy == STRATEGY_BEAM:
                ...
                starts_lm = currents.new_full(
                    (batch_size, beam_size, 1), self.lm.indexer.start_index
                )
    ...
    def encode(
        self, images: torch.Tensor, masks: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
    ...
    def init_state(self, features: torch.Tensor, lm: bool = True) -> DecoderState:
    ...
    def score(
        ...
        **kwargs: Any,
        ...
        if masks is not None and len(masks) != len(images_or_features):
            raise ValueError(
                "images_or_features and masks must have the "
        ...
        if len(images_or_features) == 1:
            ...
            raise ValueError(
                "images_or_features must have batch size 1 or "
        ...
        outputs = self(
            images_or_features, masks=masks, strategy=targets, length=length, **kwargs
        )
        ...
        indexed = self.indexer(captions, start=False, stop=True, pad=False, unk=True)
        totals = []
    ...
    def bleu(
        ...
        **kwargs: Any,
        ...
        return metrics.bleu(dataset, predictions, annotation_index=annotation_index)
    ...
    def rouge(
        ...
        **kwargs: Any,
        ...
        if predictions is None:
            predictions = self.predict(dataset, **kwargs)
        return metrics.rouge(dataset, predictions, annotation_index=annotation_index)
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        loader = data.DataLoader(
            dataset if features is None else features,
    ...
    def fit(
        ...
        max_epochs: int = 100,
        patience: int = 4,
        hold_out: Union[float, Sequence[int]] = 0.1,
        ...
        num_workers: int = 0,
        device: Optional[Device] = None,
        display_progress_as: Optional[str] = "train decoder",
    ) -> None:
        ...
        train_loader = data.DataLoader(
            WrapperDataset(train),
            num_workers=num_workers,
            batch_size=batch_size,
            shuffle=True,
        )
        val_loader = data.DataLoader(
            WrapperDataset(val), num_workers=num_workers, batch_size=batch_size
        )
        ...
        stopper = training.EarlyStopping(patience=patience, decreasing=not stop_on_bleu)

        # Wrap the batch -> loss computation in a fn since we use it for both
        # training and validation.
        def process(
            batch: Tuple[OptionalTensors, str], regularize: bool = True
        ) -> torch.Tensor:
            ...
            outputs: DecoderOutput = self(
                inputs.to(device), length=length, strategy=targets, mi=False
            )
...
class DecoderWithCLIP(Decoder):
    ...
    def forward(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        outputs = super().forward(images, masks=masks, strategy=STRATEGY_BEAM, **kwargs)
    ...
    @classmethod
    def from_decoder(cls: Type[DecoderWithCLIPT], decoder: Decoder) -> DecoderWithCLIPT:
...
def decoder(
    ...
    **kwargs: Any,
    ...
    if "tokenize" not in indexer_kwargs:
        indexer_kwargs["tokenize"] = lang.tokenizer(
            lemmatize=False, ignore_stop=False, ignore_punct=False
        )
