...
class Encoder(serialize.SerializableModule):
    ...
    @overload
    def forward(
        self, images: torch.Tensor, masks: torch.Tensor, **kwargs: Any
    ) -> torch.Tensor:
        ...

    def forward(
        self, images: torch.Tensor, masks: Optional[torch.Tensor] = None, **kwargs: Any
    ) -> torch.Tensor:
...
SpatialConvEncoderConfig = Tuple[
    ClassifierFactory, ClassifierLayers, ClassifierNumFeatures, ClassifierFeatureSize
]
...
PyramidConvEncoderConfig = Tuple[
    ClassifierFactory, ClassifierLayers, ClassifierFeatureSize
]
...
class PyramidConvEncoder(Encoder, serialize.SerializableModule):
    ...
    def forward(
        ...
        **_: Any,
        ...
        masked = []
        for fs in features:
            ms = functional.interpolate(
                masks, size=fs.shape[-2:], mode="bilinear", align_corners=False
            )
...
def parse(key: str) -> Type[Encoder]:
    ...
    return {Type.__name__: Type for Type in (SpatialConvEncoder, PyramidConvEncoder)}[
        key
    ]
