...
class LanguageModel(serialize.SerializableModule):
    ...
    def __init__(
        ...
        self.embedding = nn.Embedding(
            len(indexer), embedding_size, padding_idx=indexer.pad_index
        )
        self.lstm = nn.LSTM(
        ...
        self.output = nn.Sequential(
            nn.Linear(hidden_size, len(indexer)), nn.LogSoftmax(dim=-1)
        )
        ...
        if reduce:
            ...
            if masks is None:
                ...
                for i, j in inputs.eq(self.indexer.stop_index).nonzero():
                    masks[i, j + 1 :] = 0
            lps = (
                lps[:, :-1][idx_batch, idx_time, idx_tokens]
    ...
    def logp(
        self, sequences: StrSequence, device: Optional[Device] = None
    ) -> torch.Tensor:
        ...
        inputs = torch.tensor(
            self.indexer(sequences, start=True, stop=True, pad=True, unk=True),
            device=device,
        )
        with torch.no_grad():
    ...
    def fit(
        ...
        display_progress_as: Optional[str] = "train lm",
    ) -> None:
        ...
        class SequenceDataset(data.Dataset):
            def __init__(self, dataset: data.Dataset, annotation_index: int = 4):
                self.sequences = []
        ...
        def lossify(sequences: StrSequence) -> torch.Tensor:
            inputs = torch.tensor(
                self.indexer(sequences, start=True, stop=False, pad=True, unk=True),
                device=device,
            )
            targets = torch.tensor(
                self.indexer(sequences, start=False, stop=True, pad=True, unk=True),
                device=device,
            )
