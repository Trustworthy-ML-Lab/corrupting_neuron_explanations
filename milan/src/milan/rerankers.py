...
class CLIPHookableMultiheadAttention(nn.Module):
    ...
    def __init__(self, module: nn.MultiheadAttention):
        ...
        self.qkv = nn.Linear(
            module.in_proj_weight.shape[1], module.in_proj_weight.shape[0]
        )
    ...
    def forward(
        self, hiddens: torch.Tensor, *_args: Any, **_kwargs: Any
...
class CLIPWithMasks(nn.Module):
    ...
    def forward(
        ...
        image_inputs = images
        if resize:
            image_inputs = functional.interpolate(
                image_inputs,
        ...
        text_inputs = torch.cat([clip.tokenize(text) for text in texts]).to(
            images.device
        )
        ...
        with nethook.InstrumentedModel(self.model) as instrumented:
            if masks is not None:
                ...
                def rule(attentions: torch.Tensor) -> torch.Tensor:
                    ...
                    attentions_masked = attentions_masked.view(attentions.shape[0], -1)
                ...
                for layer in self.mask_layers:
                    instrumented.edit_layer(
                        f"visual.transformer.resblocks.{layer}.attn.softmax", rule=rule
                    )
            ...
            similarities = images_encoded[:, None].mul(texts_encoded[None]).sum(dim=-1)
            return similarities
...
class CLIPWithMasksReranker(nn.Module):
    ...
    def __init__(self, clip_with_masks: CLIPWithMasks, lam: float = 0.5):
    ...
    def forward(
        ...
        if len(images) != len(masks):
            raise ValueError(
                "images and masks batch sizes do not align: "
                f"{len(images)} vs. {len(masks)}"
            )
        if len(images) != len(texts):
            raise ValueError(
                "images and texts batch sizes do not align: "
        ...
        for b_images, b_masks, b_texts in zip(images, masks, texts):
            ...
            sim = (1.0 - lam) * sim_masked + lam * sim_unmasked
...
def reranker(lam: float = 1.0, **kwargs: Any) -> CLIPWithMasksReranker:
