...
class TopImages(NamedTuple):
    ...
    def as_masked_images_tensor(self, opacity: float = 0.75) -> torch.Tensor:
    ...
    def as_pil_images(self, opacity: float = 0.75) -> Sequence[Image.Image]:
    ...
    def as_pil_image_grid(
        self, opacity: float = 0.75, limit: Optional[int] = None, **kwargs: Any
    ) -> Image.Image:
...
class TopImagesDataset(data.Dataset):
    ...
    def __init__(
        ...
        if display_progress is not None:
            progress = tqdm(progress, desc=f"load {root.parent.name}/{root.name}")
        ...
        for layer in progress:
            ...
            if images.shape[:2] != masks.shape[:2]:
                raise ValueError(
                    f"layer {layer} masks/images have "
        ...
        self.samples = []
        for layer in layers:
            for unit, images, masks in zip(
                self.units_by_layer[layer],
                ...
                sample = TopImages(
                    layer=str(layer), unit=unit.item(), images=images, masks=masks
                )
