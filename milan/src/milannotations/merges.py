...
def merge(
    root: PathLike, source: data.Dataset, force: bool = False, image_index: int = 0
) -> None:
    ...
    for layer_dir in progress:
        ...
        for unit, ids in enumerate(ids_by_unit):
            ...
            if images_stacked.min().lt(0.0) or images_stacked.max().gt(1.0):
                raise ValueError(
                    f"while merging source images for unit {unit} "
        ...
        numpy.save(
            f"{layer_dir}/images.npy", renormalizer(torch.stack(images_by_unit)).byte()
        )
...
def maybe_merge_and_load_dataset(
    ...
    **kwargs: Any,
    ...
    if needs_merge:
        if source is None:
            raise ValueError(
                ">= 1 layers are missing missing source images "
