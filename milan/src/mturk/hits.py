...
def generate_hits_csv(
    ...
    header += [f"{image_url_column_prefix}{index + 1}" for index in range(dataset.k)]
    ...
    for layer, unit, *_ in samples:
        ...
        if len(urls) > dataset.k:
            raise ValueError(
                f"generate_urls returned {len(urls)} "
...
def strip_results_csv(
    ...
    if spellcheck:
        ...
        vocab = lang.vocab(
            [input[in_annotation_column] for input in inputs],
            tokenize=lang.tokenizer(
