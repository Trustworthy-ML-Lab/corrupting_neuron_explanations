...
def parse_yaml_config(
    yaml_file: pathlib.Path, validate_urls: bool = True, display_progress: bool = True
) -> Config:
    ...
    for question_config in question_configs:
        ...
        if answer_index < 0 or answer_index >= len(options):
            raise ValueError(
                f"answer_index {answer_index} out of bounds "
    ...
    return Config(title=title, instructions=instructions, questions=tuple(questions))
    ...
    for question_index, question in enumerate(test.questions):
        ...
        for image_index, image_url in enumerate(question.image_urls):
            questions_xml_lines.append(
                "<td>" f'<img src="{image_url}" alt="image {image_index + 1}"/>' "</td>"
            )
