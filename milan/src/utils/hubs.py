...
class ModelConfig:
    ...
    def __init__(
        ...
        state_dict_data_parallel: bool = False,
        **defaults: Any,
    ):
        ...
        self.state_dict_data_parallel = state_dict_data_parallel
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        # override path if state_dict is set
        ...
        if path is not None and load_weights:
            ...
            if self.state_dict_data_parallel:
...
class ModelHub:
    ...
    def load(
        self, name: str, path: Optional[PathLike] = None, **kwargs: Any
    ) -> nn.Module:
...
class DatasetConfig:
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        if not path.exists() and "root" in kwargs:
            path = pathlib.Path(kwargs["root"])
            kwargs.pop("root")

        if not path.exists():
            raise FileNotFoundError(f"dataset path does not exist: {path}")

        # Create the dataset, forwarding path if needed.
        dataset = factory(path, **kwargs)
        self._logger.info(
            f"Loaded dataset from {path} with kwargs {kwargs} and length {len(dataset)}"
        )
...
class DatasetHub:
    ...
    def load_all(
        self, name: str, *others: str, path: Optional[PathLike] = None, **kwargs: Any
    ) -> data.Dataset:
        ...
        for other in others:
            concated += self.load(other, path=pathlib.Path(path) / other, **kwargs)
        return concated
