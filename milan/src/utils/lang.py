...
def tokenizer(
    nlp: Optional[en.English] = None, lemmatize: bool = True, **kwargs: Any
) -> Tokenizer:
...
def vocab(
    ...
    ignore_in: Optional[StrIterable] = None,
) -> Vocab:
    ...
    tokens = [
        token for token, count in counts.most_common() if not ignore(token, count)
    ]
...
class Indexer(serialize.Serializable):
    ...
    @overload
    def __call__(self, texts: StrSequence, **kwargs: Any) -> Sequence[Sequence[int]]:
    ...
    def index(
        ...
        for tokens in [tokenized] if singleton else tokenized:
            ...
            if unk:
                indices += [self.vocab.ids.get(tok, self.unk_index) for tok in tokens]
            else:
    ...
    def unindex(
        ...
        for indices in [indexed] if singleton else indexed:
            ...
            for index in indices:
                ...
                for (special, token), keep in zip(
                    self.specials.items(), (start, stop, pad, unk)
                ):
                    if index == special:
    ...
    def reconstruct(
        self,
        inputs: Union[
            Sequence[int], Sequence[Sequence[int]], StrSequence, Sequence[StrSequence]
        ],
        ...
        for tokens in tokenized:
            ...
            text = " ".join(
                [token for token in tokens if token not in self.specials.values()]
            )
