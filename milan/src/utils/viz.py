...
def wandb_image(image: Image.Image, caption: str, **kwargs: Any) -> wandb.Image:
...
def wandb_images(
    images: Sequence[Image.Image], captions: StrSequence, **kwargs: Any
...
def random_neuron_wandb_images(
    dataset: data.Dataset[datasets.TopImages],
...
def generate_html(
    ...
    if predictions is not None and len(predictions) != length:
        raise ValueError(f"expected {length} predictions, " f"got {len(predictions)}")
