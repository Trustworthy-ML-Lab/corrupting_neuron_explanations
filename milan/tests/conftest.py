...
def top_image_tensors():
    ...
    for _ in range(N_LAYERS):
        ...
        for _ in range(N_UNITS_PER_LAYER):
            images = torch.randint(256, size=TOP_IMAGES_SHAPE, dtype=torch.uint8)
...
def top_image_masks():
    ...
    for _ in range(N_LAYERS):
        unit_masks = []
        for _ in range(N_UNITS_PER_LAYER):
            masks = torch.randint(2, size=TOP_IMAGES_MASKS_SHAPE, dtype=torch.uint8)
...
def top_images_dataset(top_images_root):
    ...
    return datasets.TopImagesDataset(
        top_images_root,
...
def top_image_annotations():
    ...
    return [
        [layer(layer_index), unit_index, annotation(layer_index, unit_index)]
...
@pytest.fixture
def annotated_top_images_dataset(top_images_root, top_images_annotations_csv_file):
