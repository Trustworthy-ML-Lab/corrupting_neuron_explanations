...
def model():
    ...
    for index in range(2, conftest.N_LAYERS + 1):
        layer = (
            f"conv_{index}",
            nn.Conv2d(
                conftest.N_UNITS_PER_LAYER, conftest.N_UNITS_PER_LAYER, 4, padding=2
            ),
        )
        layers.append(layer)
...
def test_discriminative_no_layer(
    model,
    dataset,
    results_dir,
    viz_dir,
...
def test_discriminative_units(
    model, dataset, results_dir, viz_dir, tally_cache_file, masks_cache_file
):
