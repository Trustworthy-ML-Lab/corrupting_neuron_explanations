...
def image_path_hook(image_path):
    if settings.NOISE_ATTACK:
        # remove data directory path from image_path
        data_directory_path = settings.DATA_DIRECTORY

        # get last directory name
        last_directory_name = os.path.basename(os.path.normpath(data_directory_path))
        data_directory = get_noise_attack_data_directory(
            path=last_directory_name,
            model=settings.MODEL,
            dataset=settings.DATASET,
            layer=settings.LAYER,
            noise_type=settings.NOISE_ATTACK_TYPE,
            probability=settings.NOISE_ATTACK_PROBABILITY,
            std=settings.NOISE_ATTACK_STD,
            timestamp=settings.TIMESTAMP,
        )
        new_path = os.path.join(f"../../{data_directory}/images", image_path)

        # check if image path exists
        if os.path.exists(f"{settings.DATA_DIRECTORY}/images/{new_path}"):
            logger.info(f"{new_path} exists: Using it for processing")
            return new_path
        else:
            return image_path
        ...
        # get last directory name
        last_directory_name = os.path.basename(os.path.normpath(data_directory_path))
        data_directory = get_concept_attack_data_directory(
            path=last_directory_name,
            model=settings.MODEL,
            dataset=settings.DATASET,
            layer=settings.LAYER,
...
class SegmentationData(AbstractSegmentation):
    ...
    def resolve_segmentation(cls, m, categories=None):
        ...
        for cat, d in row.items():
            ...
            if all(isinstance(data, int) for data in d):
                # create an array of shape (len(data), row['sh'], row['sw'])
                result[cat] = (
                    np.array(d, dtype=numpy.int16)
                    .repeat(row["sh"] * row["sw"])
                    .reshape((len(d), row["sh"], row["sw"]))
                )
                continue
            ...
            for i, channel in enumerate(d):
                if isinstance(channel, int):
                    ...
                    rgb = image_to_tensor(os.path.join(directory, "images", channel))
    ...
    def segmentation_data(self, category, i, c=0, full=False, out=None):
        ...
        if isinstance(channel, int):
            ...
            return out
        png = image_to_tensor(os.path.join(self.directory, "images", channel))
        if full:
...
def decode_index_dict(row):
    ...
    for key, val in row.items():
        if key in ["image"]:
            result[key] = image_path_hook(val)
        elif key in ["split"]:
            result[key] = val
...
class SegmentationPrefetcher:
    ...
    def fetch_tensor_batch(self, global_labels, model_type="pytorch"):
        """Iterator for batches as arrays of tensors."""
        batch = self.fetch_batch()
        return self.form_caffe_tensors(batch, global_labels, model_type)

    def tensor_batches(self, global_labels=False, model_type="pytorch"):
        """Returns a single batch as an array of tensors, one per category."""
        while True:
            batch = self.fetch_tensor_batch(
                global_labels=global_labels, model_type=model_type
            )
            if batch is None:
                raise StopIteration
            yield batch

    def get_image_at_index(self, index, model_type):
        """Returns the image at the given index."""
        img = self.segmentation.filename(index)
        img = image_to_tensor(img)

        if model_type == "caffe":
            img = caffe_normalize_image(img)
        else:
            img = pytorch_normalize_image(img)

        img = img.unsqueeze(0).numpy()
        return img

    def form_caffe_tensors(self, batch, global_labels, model_type):
        ...
        for record in batch:
            ...
            for c, cat in enumerate(self.categories):
                if cat == "image":
                    # Form images in the right order for model
                    if model_type == "caffe":
                        batches[c].append(caffe_normalize_image(record[cat]))
                    else:
                        batches[c].append(pytorch_normalize_image(record[cat]))
                elif global_labels:
                    batches[c].append(
...
def caffe_normalize_image(rgb_image, bgr_mean=None):
    """
...
def pytorch_normalize_image(rgb_image):
    """
    Convert from channel-last to channel-first and
    normalize to [0,1]
    """
