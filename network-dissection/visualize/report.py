...
def generate_html_summary(
    ...
    ds = fo.data
    loader = fo.get_loader(batch_size=1)
    ...
    if verbose:
        logger.info("Sorting units by score.")
    if imsize is None:
    ...
    for label_order, record in enumerate(rendered_order):
        ...
        if force or not ed.has("html/%s" % imfn):
            logger.info("Visualizing %s unit %d" % (layer, unit))
            ...
            for x, index in enumerate(top[unit]):
                row = x // gridwidth
                col = x % gridwidth
                features = fo.get_features(
                    fo.model,
                    fo.norm,
                    [loader.get_image_at_index(index, settings.MODEL_TYPE)],
                )
                features = features.detach().cpu().numpy()

                # log info about top images
                logger.info(ds.filename(index))
                logger.info(ds.image[index])

                # save the modified image directly to output folder
                imsave(
                    ed.filename("html/image/unit-%d-%d.jpg" % (unit, x)),
                    imread(ds.filename(index)),
                )

                image = imread(ds.filename(index))
                mask = imresize(features[0][unit], image.shape[:2], mode="F")
            ...
            # concat tiled and original image along dim 0
            filename_without_ext = (
                ed.filename("html/" + imfn).rsplit(".", 1)[0] + "_original"
            )
            new_path = (
                filename_without_ext
                + "."
                + ed.filename("html/" + imfn).rsplit(".", 1)[1]
            )
            imsave(new_path, numpy.concatenate((tiled_without_mask, tiled), axis=0))
        # Generate the wrapper HTML
        graytext = (
            " lowscore" if float(record["score"]) < settings.SCORE_THRESHOLD else ""
        )
        html.append(
            '><div class="unit%s" data-order="%d %d %d">'
